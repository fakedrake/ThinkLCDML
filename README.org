...
**** =module_options=
     ...
     The other way is to start the command with =custom= like and follow
     it by a comma separated list of values.
     ...
     The values are all unsigned long ints except for color-code that is
     a string (no ="=):
