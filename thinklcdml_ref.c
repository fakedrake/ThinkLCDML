...
static int tlcdml_hard_layer(struct fb_info *info, unsigned long hard_addr, unsigned long size)
{
	...
	PRINT_E("Request mem region failed for fb hard addr %p\n", (void*)hard_addr);
	return -ENOMEM;
    }
...
static int thinklcdml_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	PRINT_E("%s failed. vmode is FB_VMODE_YWRAP.\n", __FUNCTION__);
	return -EINVAL;
    }

    if(var->xoffset + var->xres > info->var.xres_virtual) {
	PRINT_E("%s failed. Screen out of virtual on x.\n", __FUNCTION__);
	return -EINVAL;
    }

    if(var->yoffset + var->yres > info->var.yres_virtual) {
	PRINT_E("%s failed. Screen out of virtual on y.\n", __FUNCTION__);
	return -EINVAL;
    }
